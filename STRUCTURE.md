...
is an IA/Wireframes artifact, not a separate stage, though it does carry its own row in the
sidebar. The shared left sidebar is generated by `wireframes/_generators/resync_sidebar.py` for the
root viz pages and by `ia_annotations.py` for the annotation pages. Neither prints a number: the
rows are labels, which is why this renumbering did not touch a single screen.
