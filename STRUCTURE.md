...
| `assets/icons.svg` | **the ONE icon sprite, 29 symbols, 18.8 KB**, added 2026-08-09. Every screen and every kit page reaches it as `<use href="../assets/icons.svg#i-name">` and no document carries a copy. It was inlined into 112 documents until that day: 1,756 KB, 23 per cent of the painted tree, half of it unused on any given screen, and one glyph had drifted into two versions. **The painted tree must be SERVED**, because an external `<use>` is a cross-document reference and `file://` blocks it |
