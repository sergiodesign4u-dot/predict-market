...
| **Futuur** | Crypto + fiat hybrid, global (Curacao license) | Closest structural analog on distribution: global, non-regulated, USDC, no US/UK/AU/FR/ES/NL users. **Called the closest analog to our MODEL until 2026-08-19 on the ground that both were AMM**, which stopped being true of us when open question 1 closed: Futuur prices along a curve, we quote and fill at the quote. 1,000+ live markets. No 2026 funding or major feature announcements found. Stagnant growth signals. Still the most relevant reference for a global, non-regulated, hybrid platform - including what NOT to do on trust. | Hybrid onboarding explanation, trust gap (14/40 in benchmark), commission model, what stagnation looks like |
...
| **Hyperliquid HIP-4** | Zero-fee on-chain prediction market, launched May 2, 2026 | New entrant with explicit zero-fee strategy as attack on Polymarket and Kalshi. Fully on-chain binary and multi-outcome contracts. First 24h: 6.05M contracts. Integrates natively with Hyperliquid perps/spot wallet. **Called the closest analog to our Web3 AMM vision until 2026-08-19**; the first release carries no chain and no commission at all (`PRODUCT.md`, Liquidity and risk), which makes Hyperliquid a reference on zero-fee sustainability rather than on architecture. Small but growing and directly competes on fees. | Zero-fee sustainability model, integration with existing DeFi wallet, cold-start liquidity approach, whether "free" is a moat or a trap |
...
**1. CLOB or AMM for MVP - RESOLVED 2026-08-19: NEITHER, the platform quotes and fills at the quote**
CLOB (Polymarket model) gives real market prices but needs liquidity from day one. AMM (Azuro/Futuur model) always works but may produce a worse user price and is harder to explain. Hyperliquid HIP-4 (May 2026) launched zero-fee on-chain contracts - free to use but revenue model unclear (relies on existing Hyperliquid ecosystem fees elsewhere). Zero-fee is not a model we can copy without Hyperliquid's existing ecosystem.

**What resolved it was not more research, it was reading the product.** This row framed the choice as CLOB against AMM and both branches assume the mechanism prices the bet. Measured over both trees on 2026-08-19, the screens do not: **18 placements over 18 documents print `shares = stake / price` to the cent, 0 of them with any slippage**, beside a line saying the price cannot move against you between the panel and the bet. An AMM fills along its curve and a book fills against whoever is there, so the tree had been drawing a third thing for months while this question stayed open. **A question left open long enough gets answered by the build**, and nothing here reads the build. The decision is a quoted price, filled whole at the quote, moved between bets by a scoring rule, with the platform as the only counterparty: `PRODUCT.md`, Liquidity and risk, and `docs/decisions.md` 2026-08-19.

**And the same read found this file was right where the summary was wrong.** `PRODUCT.md` called Polymarket an AMM until 2026-08-19; four rows of this document have said CLOB on Polygon since June. The research held and the one-line summary in the file that does not own the fact is what drifted.
...
Kalshi solves it through CFTC regulation but now has 150+ insider trading investigations (Q1 2026) and requires employer-disclosure for high-risk markets. Polymarket solved it through volume and UMA arbitration but now faces a CFTC insider trading complaint (June 2026 - Army soldier, $400K+ profit from classified intel). The implication: resolution trust is not just "who controls this?" but "can this be gamed?"

**HALF ANSWERED 2026-08-19, AND THE ANSWER CAME FROM THE OPERATING PROBLEM RATHER THAN FROM TRUST.** This row's proposed answer was "on-chain transparency + clear resolution rules", and the launch decision removed the first half: the first release carries no chain (`PRODUCT.md`, Liquidity and risk). What replaces it is stronger and it was found by asking how one person resolves 25 markets without doing it by hand: **a market is created only if a program can read its outcome from a named endpoint**, which turns the resolution record into a machine reading with a source, a raw value and a read timestamp. That is auditable by a reader in a way a team's word is not, and it answers "can this be gamed?" better than a hash does, because a hash proves a decision was recorded and a reading proves what the decision was made FROM. The unautomatable remainder is real and is capped rather than solved: measured against the shipped catalog, **6 of 25 questions are machine-resolvable from a free public source, 5 are borderline and 14 are human-only**. Those 14 keep the original question open. `ia/docs/sitemap.md`, Event and Resolution entities; `docs/decisions.md` 2026-08-19. [?] on the human-resolved remainder only
