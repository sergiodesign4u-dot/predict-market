...
| [`ui-kit/`](./ui-kit/) | **The vitrine and the gates**: a stand page per component, the token page, the icon sheet, the stage's five documents rendered as pages, and `_check_kit.py` (<!-- gates:start -->**39 gates** in 102 checks<!-- gates:end -->) |
...
| Tokens + Components | ✅ Done - two token levels, three geometry scales, <!-- counts:start -->**39 components**, 37 of them composed on three levels (5 atoms, 13 molecules, 19 organisms, computed from the markup) and 2 that are the substrate a screen stands on rather than a part of one (`base`, `course-chrome`)<!-- counts:end -->, 48 pages in the vitrine (36 stands + the token page, the icon sheet and the stage's five documents), <!-- gates:start -->**39 gates** in 102 checks<!-- gates:end -->, and a light theme as the proof of the semantic layer. Nine audit passes (7 to 9) are recorded in [`docs/decisions.md`](./docs/decisions.md); the surfaces they stopped short of are in [`docs/backlog.md`](./docs/backlog.md) |
