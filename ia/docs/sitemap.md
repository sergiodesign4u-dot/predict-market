...
- **States (WRITTEN 2026-08-18, and this line had never existed).** A view over the Event Feed carries the Event Feed's state set, because it fetches from the same place and fails the same way. States: loading (fetching your saved events) - empty (nothing saved yet: CTA to Event Feed) - error (failed to load your saved events - retry CTA). **The trees had built the first two and not the third for as long as the view has existed**, and nothing could report it, because a state that is not declared anywhere cannot be compared to what stands: the Favorites view was the only surface in this document with no `States:` line at all. `favorites-error.html` ships in both trees from 2026-08-18. **`docs/decisions.md` same date.** The declared set stops here: search is a page plus two states in `ia/docs/pages/system.md` and it has exactly those, and the Loss Screen has no error and the Wallet no empty **on purpose** - the Win error is Share Card generation (T11) and the Loss Screen has no Share Card, and a Wallet with no transactions still has a balance and a funds-safety line to show.
...
- **Logged-out header:** the balance figure and the avatar dropdown are removed and replaced by Log in + Sign up entries. Favorites (heart) and Notifications (bell) are kept as affordances, but tapping either while logged out routes to Sign In (saving and alerts need an account), and the bell shows no unread badge. **Both are DESK-ONLY from 2026-08-18** (`docs/backlog.md` 205): below the DESK rung the thumb bar carries Favorites in slot 3 and Sign in in slot 4, so neither mark adds a destination, and with the 44px touch floor on they cost the row more than the header can give at 320. The heart had been desk-only since it was written; the bell joins it, and the auth pair keeps its full width instead of being squeezed. On mobile the Portfolio bottom-nav slot becomes a Sign in entry.
