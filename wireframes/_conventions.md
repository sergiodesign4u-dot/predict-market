...
| the busy region | `<main class="feed" aria-busy>` on three screens, the inner container on the rest | `.cat-main` on those three, the same inner container on the rest | **Recorded 2026-08-16 rather than fixed, and it is the plate-wrapper row above showing its consequence.** `.cat-layout`, `.cat-main` and `.feed-inner` are paint-only, so on `my-profile`, `public-profile` and `wallet` the grey tree has no inner container to mark and marks `<main>` instead. It is the declared difference working, not drift, and it is why the loading status line went INSIDE the busy region rather than onto it: a `role="status"` on `<main>` would have taken the landmark away, and a `<p>` inside works in both trees |
