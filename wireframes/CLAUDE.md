...
123 screens, counted 2026-08-23, the last four being the cancelled-event family: an event that can no longer be settled returns every stake, and `Cancelled` had been a value of `Bet.Status` with no surface anywhere since the map was written. It was 119 earlier the same day. **It said 114 until then**, which is a typed count going stale in the file that owns the tree. It was 114 from 2026-08-18, when `favorites-error` closed the one state a view over the feed was missing, and 113 earlier the same day when `privacy`, `cookies`, `responsible-betting` and `about` arrived together as the Type 1 documents the IA had registered and nobody had built. It was 109 before them. This tree owns **structure and copy**. It decides what is on a page, in what order, and
