...
- **One state of this control is declared, is real, and has NO picture here, which is a gap in the instrument rather than in the component.** On the how-it-works head the focus ring is `--text-strong` and not the system's brass, because `.hiw-glow` puts a brass radial in exactly the corner the ring lands in: measured with the element focused, `rgb(255,255,255)` there against `rgb(215,172,83)` on every other head in the Vault, and `rgb(33,31,25)` against `rgb(104,79,24)` in daylight. The states pass cannot see it: a face is nine properties and the outline is not one of them, so the tenth occurrence joined an existing group instead of opening its own. It is named here because a gallery with no picture reads as "this control has one answer" when the stylesheet says two. `ui-kit/docs/backlog.md` S40 and `ui-kit/docs/defects.md` row 62.
