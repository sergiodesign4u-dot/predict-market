...
- `article.card @feed-grid` - The card in a grid, at rest and under the pointer. The whole card answers, the photograph does not move, and the question does not shift: a grid where every card lifts a different amount is a grid that flickers as a thumb crosses it. **The answer is the same on `article.card.skeleton`**, and that is the point of a loading card rather than an oversight: it stands exactly where the real one will, holds the same height and gives the pointer the same reply, so nothing jumps or changes its mind when the data arrives. What the skeleton is made of belongs to `skeleton` and is live on its own page. **The lift is 3px and it was invisible to this instrument until 2026-08-04**: `transform` and `box-shadow` were not among the five values a face was made of, so the record under the hover picture used to be the same string as the record under the rest picture.
...
- `article.card @card-detail-head` - The same class with **no plate at all**: inside `.ed-main` the ground, the edge, the corner, the shadow and the lift are all taken away by `components/card.css`, so what is left is a naming convention holding a layout. **The answer is the same on `div.card`** in the detail column, and the tag is the only thing that differs, which is not an anatomy. Three of the four faces on this page are the component and this one is the substrate, which is the clearest picture in the system of what inventory L75 is saying.
