...
- `button.sel @dialog.app-dialog` - **The same chip SELECTED, and it is ONE group where it was two.** Brass tint, brass edge, brass label, bold. Until 2026-08-04 the deposit sheet's copy was a solid brass GRADIENT and the bet sheet's was the tint, so the capture photographed two, and the caption that stood here described the tint on the group that was not wearing it. The gradient is gone, the two faces measure identical, and the grouping merged them without being asked. **A caption that had been quietly wrong for a stage is the sentence the merge made true.** **And it happened a second time on 2026-08-06.** S35 measured the whole chip family and found that the tint this control had just been given was itself one of TWO live spellings: 815 category chips wore `--tint-brass-09` inside a `--tint-brass-45` edge with `--text-brass-chip` and a faint halo, and 543 wore `--tint-brass-16` with `--text-brass` and none. Neither file gave a reason; both tokens carried only a provenance. The majority won, and it is also the safer pair in daylight, where the two inks collapse to one `--brass-700` and the other ground is the deeper of the two. The deposit sheet's copy and the bet sheet's split into two groups for the hour between the two halves of that edit, and merged again when the second landed - which is the gallery doing the checking rather than the caption.
