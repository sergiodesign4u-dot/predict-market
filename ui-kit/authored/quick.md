...
None, and it is declared in `ui-kit/_levels.py` STATIC. The amounts went to `components/chip.css` on 2026-08-06 as `.chip.chip-amount`, the face a chip wears when it is one of the things you fill in on a sheet, and they took the hover and the press with them. What is left in this file is a ROW: a flex line that wraps, and a bet panel pushing it to the right. A file holding one layout rule for one row is the shape `ui-kit/docs/backlog.md` S41 describes about loadmore, and this file has joined it. The gallery is on the chip page.
