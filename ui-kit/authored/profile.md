...
- `ui-kit/docs/inventory.md` L155 - "Section divider (`p.pos-status`, brass tick)", filed under `components/position.css`. **That row is false as of 2026-08-03**: `.pos-status` is styled in five rules of `components/profile.css`, including the brass tick, and `components/position.css` does not mention it once. It is the second row of this table found naming the wrong file; the first was L163, found while writing `account`, and there the file that actually draws the wallet's transaction list is `components/position.css`.
...
- `.pos-status` - the divider between the identity and the record, with its brass tick before it. The name says `pos`, the inventory files it under `components/position.css`, and it is drawn here, in five rules of `components/profile.css`. All three of those are worth knowing before touching it.
