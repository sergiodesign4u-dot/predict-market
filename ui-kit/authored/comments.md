...
- `button @tabs-detail (2)` - The other action in a comment's row, and the only square corner in this component: 0px against the 10px everything else here carries, with the edge and the ink both going brass under a press where its neighbour takes the strong ink. Two actions sitting side by side in one row and answering differently is the kind of thing a page of pictures cannot show and a page of measurements has to.
