...
- `button.bp-side.sel @.bet-panel` - The side that is chosen, in the panel, at rest, hovered, held and focused. It is filled in the outcome's colour, and the fill is the panel's loudest statement: whatever else is on the screen, this says which way the money goes.
- `button.bp-side @.bet-panel` - The side that is not chosen, all four faces. It stays legible rather than being dimmed, because changing your mind is a normal thing to do at this point and a greyed alternative reads as unavailable.
- `button.bp-side @.bet-dock` - The side that is NOT chosen, in the dock, and it is the one member of this pair that answers a pointer like a control rather than like a statement. Measured against the same class in the panel: the dock's steps to the pressed stone on hover and takes its label all the way to white, where the panel's stops one stone short and keeps the primary ink. It arrived in this gallery on 2026-08-04, when the capture started keying a face by its SCOPE as well as its class: before that the panel's copy was measured first and this one was skipped as a repeat of it.
- `button.bp-side.sel @.bet-dock` - The chosen side inside the collapsed dock at 360. The same control at a different size, and the faces are photographed separately because the dock's ground is not the panel's.
...
- `span.bp-side @.bet-panel` - The side as a STATEMENT rather than a control, in the states where the choice is already made and the panel is reporting: processing, error, resolved. Same class, no pointer, and the gallery shows the disabled reading as not-set, which is correct - it is a span and there is nothing to disable.
