...
- `button.icon-btn @button-outcome-row` - **The ghost circle, and it is one control under four names and two elements.** Photographed outside the header, where nothing else in the band is around it. **The same answer, measured across all four states in both themes, is given by `.bal-swap` (the Portfolio / Cash swap), by `.desk-only` (the control that exists only above the breakpoint) and by the `summary` that opens the bell and the avatar dropdowns.** That is not an accident anybody has to keep in step: `components/header.css` gathers `.icon-btn`, `.notif-menu summary` and `.avatar-menu summary` into ONE rest declaration, one hover and one press, after measuring the shape across 105 screens at 360 and 1440. What differs between them is the ELEMENT, because a summary opens a dropdown and a button does not, and the element is not the face. The count badge the bell carries is content inside the circle, not a state of it, and it is live on this page in the logged-in specimen.
- `button.icon-btn.bal-add @.app-header` - The plus that opens Add funds. Same circle, and deliberately not brass: the header already has one primary and it is Sign up, on the other variant. This one does NOT merge with the four above, which is the useful half of the measurement.
