...
- How it works is no longer captured here. It went to `components/button.css` on 2026-08-06 as `.btn.btn-ghost.btn-xs`, the family's third emphasis, which the census found by measuring 177 placements wearing a transparent ground. **This band has four controls in one skin - the icon button, the two disclosures and that pill - and S48 closed on 2026-08-07 by naming what they share rather than where it lives.** A skin is a set of TOKENS: a fourth file drawing three atoms is three more file-slots on the atom map and a shared class is that file with a name on it, and the two `<summary>` disclosures prove it had to be a value, because a summary is neither `button` nor `iconbutton`. So the rule stays in three files, each saying that its control stands on this band, and the band says once what it does to whatever stands there.
...
- The account menu's rows are no longer captured here. They went to `components/navitem.css` on 2026-08-06 with their ink, their hover and their press, and their picture is on the navitem page. What this file still owns inside that dropdown is the panel: its ground, its corner, its shadow and where it opens.
- The notification rows are no longer captured here either. They joined the account rows in `components/navitem.css` on 2026-08-06: the ink, the hover and the press were one selector each for both controls, split when the account row left and reunited when the two values holding this one back turned out to draw nothing. What this file keeps inside that dropdown is the panel and the row's own content, its title and its detail line.
