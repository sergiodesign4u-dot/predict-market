...
- How it works is no longer captured here. It went to `components/button.css` on 2026-08-06 as `.btn.btn-ghost.btn-xs`, the family's third emphasis, which the census found by measuring 177 placements wearing a transparent ground. This band has four controls in one skin - the icon button, the two disclosures and that pill - and after two migrations the skin is written in three files. `ui-kit/docs/backlog.md` S48.
