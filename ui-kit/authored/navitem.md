...
- `.nav-item.nav-row` - a line of a menu: full width, left aligned, 11px, `--text-primary`. The five in the account dropdown are `<a>` since 2026-08-07, which is what found the atom's missing `display`: `width:100%` does nothing to an inline box, and the `<button>` element had been supplying `inline-block` for as long as every one of these was a button.
...
- `a.nav-item.nav-row.nav-row-stack @.app-header` - **The menu row and the notification row are one control with two contents, and this is both of them.** The account menu's line and a notification's title-and-detail rest transparent at 8/8 with `--text-primary`, answer the pointer with the header's 10 per cent brass wash and press to `--bg-pressed` - and those three were literally one selector each in `components/header.css` before the migration split them. What separates the two is `display`: a notification is a block because it holds a title over a detail, and a menu row is one line - which used to mean it took the element's default and now means it takes the atom's, `display:block`, declared on 2026-08-07 when the row stopped being a `<button>` and 365 of them collapsed from 194x33 to the width of their own text. The 2px between rows is the product's density and the stand is not padded to flatter the camera, so 3px of the focus ring is shared with the row above.
