...
- `button.sel @dialog.app-dialog` - The same chip SELECTED, and this is a state carried by a class rather than by a pseudo, because the selection survives the pointer leaving. Brass tint, brass edge, brass label, bold.
- `button.sel @dialog.bet-sheet` - The selected chip inside the bet sheet, which is the same decision on the sheet's own ground.
