...
|---|---|---|
| `wireframes/` | 114 grey documents. Owns STRUCTURE and COPY. A block is decided here first | no stylesheet at all, an inline `<style>` each |
| `ui-visual/` | 115 documents, 114 of them screens and `overview.html` the index of the tree | exactly `components/index.css` |
...
in the tree: **114 of the 115 documents in `ui-visual/` link `../components/index.css` and nothing
...
`transition`, `animation` and `@keyframes` stand **0 times in the 115 painted screens** and 0 times
