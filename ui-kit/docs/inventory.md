...
| `search` | 17 | **ONE CONTROL.** A field that holds a query, the mark inside it, the control that empties it and the mono line that answers. Written 2026-08-16, and it is **the first component in this system added for a control the IA had DECIDED against**: `ia/docs/sitemap.md` deferred search "until catalog scale", the catalog was re-measured at 25 events and the premise held, and what overturned the deferral was reachability instead. **It is not a variant of `input` on three counts of anatomy**: the mark is INSIDE this control and beside that one, the value is a query rather than a quantity, and this is the only field in the product a person can empty, so `.search-clear` is a state the input family has no vocabulary for |
...
| `search` | none | 17 on 3 | **FILLS `.cat-main`**, 328 at 390 and 922 at 1280, one slot and one behaviour: the field is the width of the column it stands in at every one of the thirteen. **`.search-clear` is FIXED 44x44**, which is the base floor and not this file's number, and `.search-ic` FIXED 18. The mark and the clear control are absolutely positioned inside the field and the field pads itself out of their way, so neither is a flex item whose width is what is left |
