...
| `market` | 200 | **HOLDS its own rows.** Same blind spot: a block of question, rule and source, built from its own names. **200 counted 7 names that styled a table the product does not have**, gone 2026-08-18 |
...
| `seo-plate` | 15 | **HOLDS its own prose blocks.** **15 documents and 73 blocks, re-counted from the rendered DOM 2026-08-20**; the plate itself (`.feed-seo-wrap`) is on 10, the ten feed screens, and the other five hold the blocks bare in the slot. It was 74 until About lost `#regulation`, a section that appeared in neither `ia/docs/blocks.md` nor the STATEMENT heading list of `ia/docs/pages/seo.md` and whose subject `PRODUCT.md` marks `[?]`. **A SECOND FACE ARRIVED THE SAME DAY**, `.seo-figures` with 3 `.seo-fig`, on About in both trees and nowhere else: B17's numbers block, which had shipped as two paragraphs describing what the figures would be. `docs/backlog.md` 214. It said 9 while the tree had one document in it |
...
| `notice` | 96 | **HOLDS** `button`. Filed as an organism. **`.inline-error` joined the error family on 2026-08-16 and had never been in it**: `.state-block.state-problem` and `.toast.toast-error` both took `--border-notice`, one danger triangle and a 600 message, and this box took the plain hairline, 0 marks and 400, on the surface where a bet fails  **AND `.quiet-box` GAINED ITS FIRST FACE ON 2026-08-16, `.qb-working`, AFTER THE PLACEMENTS WERE COUNTED RATHER THAN THE NAME READ.** The box is worn by three jobs: FIVE are a process running with an unknown end (the OAuth redirect, the share card, the resolution fetch, the on-chain register, and the Transak wait, which was wearing `.widget-box`), TWO are an out-of-band pending state where nothing is running in this tab (`deposit-pending`, `win-payout-pending`), and TWO are a DISCLAIMER that never finishes (the prototype note on `terms.html` and `ui-kit/browse-shell.html`). So the cycle is on a compound selector: a progress bar on a legal notice is what the bare class would have drawn. The mark is a 2px rule at `--size-2` sweeping on `--pulse-period`, the SAME period as `sk-pulse` because a spinner and a skeleton are the same STATUS job, and at `prefers-reduced-motion` it stops and becomes full width rather than shortening. Measured on both engines: translate moves 0 to ~77-82 per cent inside 350ms normally, and reads `animation:none` with a 346px bar under the setting, while the four quiet placements gained no `::before` at all. The NAME is still wrong for four of nine placements and that is `docs/backlog.md` 170.  **AND IT GAINED `.reading` ON 2026-08-19, THE ONLY FACE IN THIS FILE THAT IS DELIBERATELY UNTINTED.** `ia/docs/sitemap.md` gave the Resolution entity a **Reading** the same day and named three surfaces, so the face stands on **10 documents, 5 painted and 5 grey**, re-counted from the rendered DOM on 2026-08-20 when the multi half of the outcome family landed, and it is a face of this component rather than a component because what it adds is a content shape and not a drawing. **The colour is the argument**: every other box here is tinted with `--color-trust` or brass because it makes a promise, and a Reading reports what was read, from where and when, for the reader to check themselves; tinting it would borrow the authority the sentence exists to avoid needing. **Both of its two wordings are placed**, `read by the Yonder team` on the market only a person can settle and `read automatically` on the one a program settles, which is the rule the chosen NO cost this repository. **AND ITS THIRD SENTENCE ARRIVED THE SAME DAY IT WAS RULED OUT AS A FACE**: `loss-multi.html` carries a reading that compares figures ACROSS options, `Ethereum held $73.4B in stablecoins against Tron at $61.2B and Solana at $12.8B`, which is the shape a multi machine read has and a binary one cannot, and it arrived as DATA inside the same drawing. The figure-against-threshold wording was filed as an unmade placement and **2026-08-20 found it is not a face at all**: **148 feed cards over 15 painted documents route to the specimen of their TYPE, 111 binary and 37 multi, and 0 to their own market**, so a numeric market gets no document of its own and its sentence is data inside the same drawing. `docs/backlog.md` 221 struck; what that measurement did find is 222, one row that CANNOT obey the convention because the outcome family has no multi member  **AND `.reconcile-box` GAINED ITS THIRD FACE ON 2026-08-23, `.rec-returned`, WHICH IS THE ONE THAT TAKES NO COLOUR.** The cancelled-event family returns every stake, so the box holds a figure that is neither a win nor a loss, and `DESIGN.md` decides twice that green and red are outcome semantics: the face therefore adds a font size and nothing else, taking the brass edge, brass tint and `--text-brass` figure from the base box and the centred display arrangement from its two siblings. It stands on `cancelled`, `cancelled-multi` and `cancelled-refund-pending` in the paint, the same three in the grey where the modifier carries no rule, and the specimen on `ui-kit/notice.html`. **The `96` in the column beside this row is a reading dated before it and is not re-typed here** |
...
| `toc` | 4 | **HOLDS its own rows**, and they are anchors rather than buttons. Walked 2026-08-08. A rail above 900 and a disclosure below it. **This cell said 1 and `14 of them` until 2026-08-19**: four documents carry a contents now (terms, privacy, cookies, responsible-betting) with **47 rows between them**, and 14 was terms' own number standing in for the component's, which is the same shape as a verdict taken on one placement. `about.html` is the fifth document and carries NO contents by decision: `ia/docs/blocks.md` gives it the STATEMENT profile and B5 is DOC only |
...
| `betpanel` | 124 | **HOLDS** `yesno`, `chip`, `button`, `input`, `quick`, `notice`, `state-block`. **120 until 2026-08-17**: `.line.total` went with the single `Potential payout` figure it drew, and `.bp-outcomes`, `.bp-outcome`, `.bp-outcome b`, `.bp-pick` and the panel-scoped groove for the pair came in with it. The arithmetic is the real surfaces only, because the HIW still-life counts under `hiw`: minus 8 `Price now`, minus 8 `Potential payout`, plus 8 `.bp-pick`, plus 4 `.bp-outcomes` and 8 `.bp-outcome` on the two screens where a side is picked. docs/backlog.md 180 |
...
| `browse-shell` | 900 | 85 documents | **FILLS** both slots on the 80 browse screens, and on the five documents it does the opposite. **A SECOND 900 RULE ARRIVED 2026-08-19** and it is about the shell's own width, which the second half of this cell used to deny: `.cat-layout:has(.read-col){width:fit-content;margin-inline:auto}`, so the plate stops where the document stops. Measured at 1440: **795 on terms** (214 rail + 20 gap + 503 column + 2x29 inset) and **561 on about**, against 1140 before, with 144px of nothing between the rail and the column and 153 to the right of it. The reason is quoted from `ia/docs/blocks.md`: on desktop the contents is promoted to a sticky left column and *the body keeps its 60 to 75 character measure rather than filling the remaining width*, so **the width is DECLARED to go unused and a border may not be drawn around it**. Below RAIL the plate FILLS as before, 292 at 320 to 819 at 899 |
...
| `hiw` | 900 | 1,391 on 105 | `.hiw-sec` 32-717 and `.hiw-faq` are INTRINSIC; `.hiw-cols` CHANGES its share at 900, where the side column arrives. **`.hiw-body` is gone from the dialog on 2026-08-14 and so is the "FILL at 414 from 640 up" that named it**: the dialog is a three-step stepper now, `.hiw-steps` FILLS the sheet at every width, and the two floors under it were FIXED, a 280px stage and a 172px text block, and **neither floor did what its comment said**: re-measured 2026-08-20 in Chromium 151 and WebKit 26.5 at 390 and 1280, the sheet stood **563, 586.5 and 610.5**, the picture frame **280, 303.5 and 280**, and Next moved 452 to 475.5 and back. The three steps share one grid cell since that date, the frame is `min-height:19rem` and the third step's extra row is reserved on the other two, so the sheet is **one height on all three steps, 634.5 at 390 and 635.5 at 1280, with the button at 476 and the dots at 583.5 everywhere**. **The stage is the one place in the system where three other components stand as PICTURES**, `card`, `betpanel` and `position`, with no focusable element in any of them, so this row still counts none of their elements as this component's |
...
| `market` | none | 369 on 9 | **FILLS** six of eight slots; `.market-head` INTRINSIC 16-177.4. **`.md-row` was in this row and it was a reading of the SHELF, not of the product**: the slippage ladder it belonged to stood on 0 of the 9 placements, and its seven rules went on 2026-08-18 with the reason in `components/market.css`. `docs/backlog.md` 211 |
...
| `notice` | none | 114 documents | **FILLS** the sheet body, 368-597; one slot in `.bp-inner` is GONE below 760; `.push-banner` CHANGES its share. **`.protect-page` was FIXED at `--measure` and it is FIXED on one screen now**, 2026-08-19: on `wallet.html` it still caps, because there the sentence stands in `.cat-main`, a column sized for cards and 150ch wide, and on the five document pages the cap comes off because the COLUMN is the measure. Measured across all five: 258 / 298 / 328 / **503.4 from DESK 640 up**, the same number as every sibling in the stack. It was 346 while the column was 600, which is what a second measure inside a reading column does |
...
| `seo-plate` | 760 | 73 on 9, and `.seo-figures` is `auto-fit` from 130px so it needs no rung of its own | `.seo-brand` and `.feed-inner` FILL; **`.feed-seo-wrap` CHANGES its share at 760**, where one column becomes two; the heading FIXED 22 |
...
| `toc` | 900 | 4 documents | **FILLS, and the container is what moves**: measured on all four 2026-08-19, **258 at 320, 328 at 390, 578 at 640, 622 at 760, 761 at 899 and 214 from 900 up**, which is the rail arriving. The link marker is FIXED 13.2. It read `31 on 1` when terms was the only document |
...
| `hiw` | response + **arrival** | the step dot answers the press; the step itself ARRIVES, 2026-08-20. The three panes stack in one grid cell and cross-fade, the text travelling `--space-32` and the still four tenths of it, both through `--hiw-shift` and `--motion`. **The nav is the one part that does not fade**, because two brass buttons crossing at half opacity composite to 75 per cent and the control a thumb is aimed at may not dim |
