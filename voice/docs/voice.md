...
| An event that can no longer be settled | **cancelled** (the event), **returned** (the money), and the chip and figure label **Returned** | void, voided, annulled, refund, reimbursed, reversed, busted | **Added 2026-08-23** with the cancelled-event screens, because the product had a Status a screen could not say. Two words rather than one, because two different things happen: the EVENT is cancelled, which is the word `ia/docs/sitemap.md` has used for the Event and Bet status since it was written, and the MONEY is returned, which is the word the product already uses for it - `$5.00 stake returned` has stood on the Win screen since it was drawn, so `returned` is not a new word here, it is the existing one reaching the one place it is the whole story. `void` and `annulled` are the sportsbook's and the lawyer's; `refund` is a shop's word for a purchase a person changed their mind about, and nobody changed their mind. And it is never `reversed`: on-chain transfers are final, which is a sentence the Wallet already makes, so a word that says the chain went backwards contradicts it. **The state FILE is `cancelled-refund-pending.html` and that is this table's own invariant, not an exception to it**: the ban is about PLACE, and a state name is read by whoever builds the screen and by nobody who uses it. `refund` is also the noun `ia/docs/sitemap.md` used for this flow while it was deferred, so the file name agrees with the map and no string a reader meets carries the word. |
