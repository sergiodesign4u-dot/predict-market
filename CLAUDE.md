...
- **Event Detail content tabs:** below the event content, a Polymarket-style tab strip (CSS-only radio switch, no JS) - **Comments** (sort + composer with likes/replies; logged-out prompts sign-in), **Top Holders** (YES/NO columns), **Positions** (table; your row highlighted when logged in), **Activity** (trade feed). On binary + multi + resolved, logged-in and logged-out. Built in `gen_event_detail.py` (page-local CSS).
