...
- **One system.** `components/` holds `tokens.css` plus one file per component, reached through
  ...
  tree links no stylesheet at all**: 124 of 124 in `ui-visual/` and 61 of 61 in `ui-kit/` link it,
  **0 of the 123 in `wireframes/` do**, and those 123 carry an inline `<style>` block each. Counted 2026-08-23 with the four
  cancelled-event states, before that 2026-08-20 at 120 / 119 and before that
...
- **"Both widths" is not a measurement of a responsive system: measure AT the rungs and one pixel
  ...
  true thing and every number in this repository has to say which**: `ui-visual/` holds 124
  documents, 123 of them screens, counted 2026-08-23, and `overview.html` is the index of the tree rather than a screen
  ...
  gave a multi resolution a page of its own: the grey tree is 123 since the cancelled-event family landed on 2026-08-23, so the two still differ by exactly the
