...
- **Chalk** (the pale stone, 12 steps `#fdfbf5` to `#dcdad4`, plus one hairline at `#acaaa4`): the graphite ramp **reflected about its own ground**, `chalk L* = page L* - (graphite step L* - graphite page L*)`. In the Vault the page is the darkest thing on screen and every surface rises off it toward the light; reflected, the page is the lightest thing on screen and every surface settles onto it by getting darker. Each step is within 0.2 L* of its computed target and carries the number of the graphite step it answers to, so `--chalk-850` answers `--graphite-850`. Nearly **neutral**: R minus B is a constant +8, the same faint offset the graphite carries in the other direction. The warmth of the Vault is in the ink and the metal, not in the stone.
