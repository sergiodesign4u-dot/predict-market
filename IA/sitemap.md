...
- Multi-outcome: N options, each row is an option plus its % plus compact YES / NO controls. On the feed card only the two leading positions are shown (the options with the highest probability / most stake at that moment), followed by a "+N more" link; the full option list appears after the user opens the event. This caps multi-outcome cards to roughly binary-card height so the feed reads evenly. The two-position cap is a feed preview rule only, it does not change Event.Type or how many options the event actually has.
