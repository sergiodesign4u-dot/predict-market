...
**THE PRODUCT HAD FIVE DURATIONS AND NOT ONE WAS A LITERAL, SO THE WORK WAS NEVER TIDYING LOOSE NUMBERS.** Measured by computed style over 163 documents in Chromium 151 and WebKit 26.5, 652 loads, both engines agreeing element for element: 4,904 moving elements on the 105 painted screens at `.16s` 9,528 slots, `.12s` 2,346, `.18s` 743, `.3s` 636 and `.25s` 63. **The drift was in the CURVE**: 12,821 of 13,406 easing slots were the bare keyword `ease`, typed out 54 times and defaulted the rest, so **95.6 per cent read no token at all**, and one of the two declared curves rendered zero times in the product because its only reader was the review harness. **And one ROLE wore four numbers**: a hover is 160ms on a button, 180 on a photo tile, 250 on a trust plate and 300 on a card, which is the stage's textbook defect in the textbook's own shape and shows only when the readings are grouped by job. 69 declarations were read, given a job and rewritten; five old token names lived one step as aliases and were deleted after the last reader moved. **The rewrite skipped every comment**, so the historical quotes in `card.css`, `chip.css` and this file still say what was true when they were written.

**THE STATUS JOB WAS PERFORMED NOWHERE, ON NINETEEN SCREENS ABOUT WAITING.** `skeleton.css` was 55 lines of flat `background:var(--bg-control)`: zero animations, zero transitions, and **482 marks on 19 loading screens, identically in both trees**. A plate that does not move is not distinguishable from an empty block, which is what a person reads from a screen that has stalled. It is `sk-pulse` now, an `opacity` cycle, opacity because it is one of the two properties a browser animates without laying the page out again and a feed skeleton is 42 marks at once. Amplitude .55 and shallow on purpose: a plate that goes nearly invisible reads as content flashing rather than as a wait. **The page that documents it had predicted this**: `ui-kit/skeleton.html` said "nothing shimmers, and it is an absence rather than a decision, because nobody has argued it either way".

**SIX COMPONENTS HAD A STATE AND NO MOVEMENT, AND TWO OF THEM ARE 2,064 PLACEMENTS.** `navitem` at 1,068 on 105 screens and `position` at 996 on 105 had hover and press declared with tokens in both themes and no transition between them, so the change happened on the frame the pointer arrived and the control never answered, it just WAS different. Plus `state-block` at 152, `footer` on every screen, `cookie-consent` at 33 and `toc`. Three refusals with reasons: the footer's underline stays instant because a line fading in from nothing reads as a rendering fault; the cookie checkbox gets nothing because its state moves `accent-color`, which is not animatable and would be a rule that never fires; and **the focus ring stays instant**, because a person moving through a form at speed passes an element in less time than any duration worth writing.

**THE CHECK FOR LESS MOTION COULD NOT FAIL, AND THAT IS THE FINDING THIS STAGE WOULD KEEP IF IT COULD KEEP ONE.** `base.css` carried `*,*::before,*::after{transition-duration:.01ms!important; animation-duration:.01ms!important; animation-iteration-count:1!important}` under `prefers-reduced-motion`, written when three components carried their own block and twenty did not, and correct on that day. By this stage it was hiding the thing it looked for: under `!important` on `*`, a component that reads no token is indistinguishable from one that reads every token. **Measured with it in place: 6,555 moving elements per engine normally against 115,028 under the setting**, which is the whole DOM taking a duration from one rule, and 0 defects out of 230,056 rows. **Taken off, with a positive control first** (an injected `transition:opacity 999ms linear` read back 999ms in both engines, so the probe can see a rule that disobeys): **1,392 elements above 1ms, and 0 of them in `components/`.** Every one was the STAND, a `.15s` literal in `ui-kit/_page.css` reaching 690 elements per engine over ten kit pages plus `ui-visual/overview.html`, and the demonstration pulse on `motion.html`. Both repaired at source, second reading **0**. **The net is deleted rather than restored**: its only argument left was code that does not exist yet, and this repository already refuses to pay rent on a decision nobody has taken.

**WHAT THE TOKEN OVERRIDE DOES NOT REACH, DECLARED AS A LIST BECAUSE A LIST IS WORTH MORE THAN A RULE THAT MAKES THE LIST UNREADABLE.** A cycle, which is REPLACED and never shortened, because 1ms per period at infinite repeats is a flicker and worse than the still box it replaces. The grey tree, **105 elements above 1ms over 105 of 105 documents**, because it links no stylesheet and a grey file cannot hold a token. The six course documents that carry their own inline `.15s`, `.2s` and one `.4s`, for the same reason in a different tree. And a cross-document view transition, which is one of the four reasons there is not one.

**THE CROSSING BETWEEN DOCUMENTS IS A, AND IT WAS MEASURED BEFORE IT WAS CHOSEN.** Two test documents with `@view-transition { navigation: auto }`, navigated for real, asking whether `pagereveal` arrived carrying a transition: **Chromium fires over http and from disk; WebKit fires over http and NOT from disk**, which is the same opaque-origin rule that has already cost this repository a sprite, a mask and two fonts. Refused on four grounds, in order: a cross-fade points at no source, and the connection job is defined as showing what came from what; doing it properly needs a shared `view-transition-name` on the card and the detail head, unique per document, on a feed that ships up to nine cards, so the name has to be written at click time; it would add an obligation the tokens cannot reach; and on the path this repository actually reads from, one engine of two would not show it. The line is one line and the measurement is banked.

**TWO MORE RULINGS, BOTH NO.** A rung does not animate its own rebuild, including for the two surfaces that qualify for the exception, the bet panel arriving at DETAIL and the sub-category rail at RAIL: both arrive only while somebody drags a window edge, and **an entrance nobody can be present for is not a connection, it is a frame that happens to differ**. And the scroll edge fade stays and stays out of the register: it binds a mask to the element's own scroll rather than to time, reads no duration, and is driven by the reader's own hand. Measured at three scroll positions, both engines, both settings, **the mask moves in all four combinations including under the setting**, because a scroll timeline takes progress from distance and not from duration. Movement a person makes themselves is not movement done to them.

**MOTION WAS THE SEVENTH BOUNDARY DIFFERENCE BETWEEN THE TREES AND NOBODY HAD DECLARED IT.** `wireframes/_conventions.md` declared six differences that ARE the boundary rather than drift, and motion was not among them, so the day the paint took its durations from tokens the two trees began to disagree about something unwritten, which is drift by that document's own definition. It is declared rather than ported, for a mechanical reason: a grey file links no stylesheet, so the numbers would have to be written 105 times, which is exactly what one width change already cost that folder in 312 edits.

**THE FRAME-COST PASS: TWO CONVERTED, ONE REFUSED, FIVE KEPT WITH A COUNT.** The toggle knob came off `left` onto `translateX(16px)`, geometry identical to the pixel at 6 and 22 in both engines. The how-it-works dot came off `width` onto `scaleX(1.75)`, 28 over 16 exactly, with the cost named: scaling a pill scales its end caps, so the selected dot's go from a 2px radius to 3.5 on a bar 4px tall. **`max-height` on the condensed band is the one refusal**, because there is no transform that removes a box from the flow and that band's whole job is to stop taking room; the cost is one element relaid for 250ms, once. And five `box-shadow` are kept because **all five sit on the element's own hover or focus, so at most one element in the document animates a shadow at a time**. `transition: all` is 0 and `will-change` is 0, and both were 0 before the stage as well.

**THE TWO-VIEWPORT WALK FOUND A HOLE THE ONE-WIDTH WALK COULD NOT.** `sheet-rise` lives inside a `max-width:39.99875rem` block, so it is the PHONE's answer, and above the desk rung **337 dialogs on 105 screens simply appeared, backdrop and all, on one frame**, with the inventory row marked done. Direction and amplitude depend on width and duration does not, which is the whole reason the closing sweep walks 360 and the desk rather than one of them. The desk modal takes `sheet-appear` now, opacity only and no travel, **because a centred modal has no edge to come from and an invented direction points at nothing**; the backdrop takes the same rule so the scrim does not land a frame before the surface it belongs to.

**THE INSTRUMENT WAS WRONG FOUR TIMES BEFORE ITS NUMBERS WERE BELIEVED, AND EVERY ONE IS THE SAME SHAPE.** A naive count of `transform` in the grey tree returned 2,263 of which **1,750 were `text-transform`**, and `animation` returned 14 of which 0 were declarations. The scroll edge fade read identical at every scroll position in both engines, which looks exactly like a feature that does not work, and two frames later reads as moving in all four combinations: **a scroll is not a paint any more than a resize is a layout**. The toggle knob read `translateX(0)` in both faces, then 0.56px in one engine and 3.82 in the other, because the probe was reading a transition in flight. And the step dot read `transform: none` on all nine of its placements on `how-it-works.html`, which is nine elements that **render at no width at all** because they live inside the shut dialog: **a computed value on a box that was never laid out is not a reading of the rule**.

**RESULT.** 9,084 moving elements on the 105 screens, **two durations rendered** (`.16s` 19,349 slots, `.25s` 171) and one period, two curves, **0 duration literals and 0 bare easing keywords in 54 stylesheets**, `sk-pulse` on 482 marks, `edge-fade` on 90. **20 roles grouped by job and every one carries exactly one number at both widths.** 0 horizontally scrolling documents at 360 and 1280 in both engines with the positive control at 3,640 and 2,940. 0 of 163 documents above 1ms under the setting with the net gone. Idle control clean both ways: 6 motion tokens declared, 6 read, 0 either way. 0 springs, 0 transitions written inside a state rule, **0 of 106 screen files carrying motion**, `wireframes/` unchanged by a single character. `ui-kit/motion.html` is the sixth foundation and the 58th page, and it is the one page in the kit that cannot be printed.

**Backlog: 12 closed as NEVER** (a live odds delta needs a delta, and no figure in any of the 105 documents changes after load, so there is no moment and no inventory row could be written), **3 re-owned to Handoff** (it is a SOURCE audit and this stage's method is a rendered one: `showModal()` was called to see two keyframes the scripts open, and nothing else about the scripts mattered). Four moments stay open and are named: the expansion of 349 `<details>`, the contents rail, and the toast and Load more, which need a state before they can have a movement.
