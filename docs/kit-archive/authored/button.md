...
- `button.btn.btn-ghost.btn-xs @button-matrix` - **The third emphasis, added 2026-08-06 because the product already wore it 105 times.** A transparent ground on a hairline pill with the quiet ink: it keeps the box and gives up the fill, which puts it a real step below secondary. Secondary has a ground and says *you may*; this has none and says *if you want*. Its hover is `--tint-brass-16`, the wash the whole header band answers with, not `--bg-control-hover`, because a fill appearing out of nothing is not a quieter version of a ground moving - it is a different event. **All 105 of these stand in `<header>`, so this emphasis is the band's**, and the wash it reads became a token on 2026-08-07 when S48 closed: it had been typed out longhand at 14 per cent in three files, a rung the brass ladder does not have, invisible to the rescale map for as long as it existed because that map matches token names. The ink lifts to primary, since ghost rests at muted and has somewhere to go.
...
- `button.btn.btn-prompt.btn-lg.btn-block @button-matrix` - **The fifth emphasis, and the only one in this family that is an INVITATION rather than an action.** A full-width box on `rgb(13, 15, 18)`, the well, inside a hairline, with `rgb(164, 157, 143)` muted text: it stands where the thing a signed-out person came for should have been - a comment composer, a bets table - and says how to get it. It is the only emphasis drawn on a RECESSED ground and the only one muted at rest, both on purpose, because a prompt that shouts is a wall. **It lights all three at once under a pointer** - edge to brass, label to `rgb(215, 172, 83)`, ground one step up to `rgb(20, 22, 25)` - which no other emphasis here does, and it is the right answer for the one control that starts quiet: a single step would leave it looking almost as it was. The press settles onto `rgb(25, 27, 31)`, the system's one quiet pressed ground. It arrived on 2026-08-07 from `components/comments.css`, and it was the last file-slot on the atom map: `ui-kit/docs/atoms.md` reached a distance of 0 when it moved.
