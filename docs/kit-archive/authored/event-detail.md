...
None, and it is declared in `ui-kit/_levels.py` STATIC. The three marks in the corner of the head answered every pointer this file used to be measured on, and on 2026-08-06 they went to `components/iconbtn.css` with their hover, their press and their favourited state, which is `ui-kit/docs/atoms.md`'s rule that a region says where a control sits and never what it looks like. What is left here is the head, the facts strip, the sections and the resolution block: a surface and a rhythm, and a surface is not pressed.

The face they wear is now the atom's fourth and it is photographed on the iconbtn page, in this component's own specimen, which is the arrangement the migration was for: the picture is taken where the control STANDS and filed where the control IS.
