...
| 29 | **Step 14 audited a LIST OF TERMS instead of an invariant, and every line that was not on the list is still standing.** Owner: **Stage 09 (Design System)**, step 8, the copy half. Found by the authored-source fan-out, round 3, 2026-08-03 | **What is standing, measured as visible text in BOTH trees, and the two counts agree, so gate 18 is quiet and correct:** `Liquidity` as a headline figure in `.ms-val` (**9 occurrences on 9 screens per tree**); `order book` in `.md-sub` (**9 on 9**); `AMM` (**14 on 10** grey, **18 on 10** painted: the `.md-sub` gloss, the `.fine` line on the detail, and How It Works); `Holders` as a sort segment inside the Comments tab (**9 on 9**), under a strip Step 14 itself renamed to *Biggest bets* / *Bets*; and non-brand `Market` (**18 on 9 screens per tree** - the `.market-title` head and the *Market Context* rules tab; the 296 and 459 remaining hits are the brand name *Predict Market* and are excluded). `voice/docs/voice.md` puts *liquidity as a headline number*, *order book*, *AMM / CLOB* and *"market" for the event* on the **Not allowed in the UI** list. **THE CLASS, and this is the third time in this project:** (1) the screen map lived as five hand-written copies instead of one computation, so 32 grey category screens sat against 4 painted ones and every unpaired page was skipped in silence, which is what `_twins.py` and gate 18 exist for; (2) `voice/docs/microcopy.md` Step 24, where the table that IS the source of truth for copy took in 43 shipped strings it had no row for, because it enumerated rows and nothing checked the invariant "a UI string gets a row before it ships"; (3) this one. Step 14 declares *"Clean on cents/spread/liquidity/order book/buy-sell"* and left every instance outside its own list, **including the same `.fine` line it edited in that pass** - it trimmed a leaked spec-note from the tail and left the `(AMM)`. **What step 8 has to know before it starts:** the fix is not "apply the rule", because `microcopy.md` Step 24 logged four of these five as ANSWERING principles 1 and 3, so one of the two documents is wrong and that is a decision, not an edit. And the grey tree is **not frozen against a text fix, it is frozen against REGENERATION**: `CLAUDE.md` forbids `wireframes/_generators/gen_*.py`, and the Stage-04 precedent is an idempotent in-place post-processor beside `footer_reconcile.py`, with the same edit in `ui-visual/` because gate 18 compares `<main>` |
