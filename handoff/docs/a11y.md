...
| 13 | One heading level one per screen, no level skipped | decided in the structure layer, carried into the paint | walk the heading levels of each document | **confirmed** as part of the earlier structural passes and **not re-run here**; it is a structural property and this stage does not read the structure tree. Named so the next reader knows which instrument owns it |
...
Three rows, each with the way to check it, each going to `docs/backlog.md` under this stage.
