...
**Account limits arrived on 2026-08-23 and is described below**, under the surfaces no flow names. It has no row in `ia/docs/flows.md`: it is reached from the account hub and from section 4 of Responsible betting, and it closes no step of any of the five flows, so it is an S3 surface like Wallet and the five documents. Its states are drawn, so it is S1 as well. **Its one missing state is filed rather than described**: `docs/backlog.md` 240, no validation surface for a rejected value, which is a gap in the family and not a decision about it.
...
| Account limits | A deposit ceiling per 7 days, a loss ceiling per 7 days, a cooldown and self-exclusion, all on one screen. **The behaviour is an asymmetry and not a form**: tightening a limit takes effect at once, loosening one or lifting it takes 24 hours, so a limit cannot be undone in the moment a person most wants to undo it. A cooldown holds the form and says why. **What no limit touches**: bets already held resolve and pay out on their own dates, and withdrawal is open throughout. Self-exclusion is a confirm dialog on this screen, longer than a cooldown and not reversible inside its period. The error state says the limits already set are still in force, because a limits screen that fails silently reads as a limit that lapsed. | `account-limits`, `-active`, `-cooldown`, `-loading`, `-error` | S1, S3 |
...
| Deposit limit, loss limit | A floor of one dollar, two decimal places, and **a direction**: a lower number applies at once, a higher one after 24 hours. The direction is the rule; the floor is the ordinary one. | `data-min` and `pattern` on the amount field of the `input` component; the waiting change is drawn as its own row on `account-limits-active` with the instant it lands and a control that cancels it | `ia/docs/sitemap.md` > ACCOUNT LIMITS | S1, S3 |
| Cooldown period | One of three named periods, never a free date. A cooldown cannot be ended early, by the reader or by us. | the `chip` component in a `quick` group; the held Save is `aria-disabled` and points at the sentence that says why, through `aria-describedby`, exactly as the bet panel's Confirm does | `ia/docs/sitemap.md` > ACCOUNT LIMITS | S1, S3 |
